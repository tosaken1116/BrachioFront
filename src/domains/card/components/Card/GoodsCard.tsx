...
import type { GoodsMasterType } from "../../types";
import { CardBase } from "./Card";

type Props = {
	card: GoodsMasterType;
};
export const GoodsCard: FC<Props> = ({ card }) => {
	const { masterCardId, name, text, imageUrl, rarity } = card;
	return (
		<CardBase card={card}>
			<div className="flex flex-col h-full bg-gradient-to-r from-blue-200 rounded-xs via-white  to-blue-200">
				<div className="flex flex-row italic justify-between rounded-[3px] bg-gradient-to-r from-blue-300 via-20% via-slate-200 to-slate-200 drop-shadow-sm py-0.25 px-1 border border-slate-300">
					<strong className="text-[6px]">グッズ</strong>
				...
				<div className="flex flex-row  h-6 items-center relative">
					...
					<div className="absolute right-0 rounded-[3px] h-4 -top-1 items-center flex bg-gradient-to-r from-blue-400 shadow-inner border  border-r-0 rounded-br-none rounded-tr-none w-3/4 border-slate-300  via-slate-200 to-blue-400 drop-shadow-sm py-0.25 px-1 text-[4px]">
